...
LEFT JOIN post as post2
ON post.id = post2.parent
WHERE post.parent IS NULL AND post2.id IS NULL AND (
    post.sender IN (
    	SELECT sender as id FROM `friends` WHERE recipient = ?
		UNION
		SELECT recipient as id FROM friends WHERE sender = ?
